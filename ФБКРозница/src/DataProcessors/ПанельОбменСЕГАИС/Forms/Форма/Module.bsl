...
Процедура ОбновитьСпискиДокументов()
	...
	ТаблицаДокументы.Колонки.Добавить("ЕстьПравоНаЧтение");
	...
	Для Каждого ТекЭлемент Из ТаблицаДокументы Цикл
		...
		Если Не ТекЭлемент.ЕстьПравоНаЧтение Тогда
	...
	Для ТекИндекс = 0 По ТаблицаДокументы.Количество() - 1 Цикл
		...
		Если ТекЭлемент.ЕстьПравоНаЧтение Тогда
			...
			Если ТекЭлемент.Оформите Тогда
				Если ТекЭлемент.Метаданные = Метаданные.Документы.ПередачаВРегистр2ЕГАИС Тогда
					...
					ВывестиПоказатель(
						...
						Элементы["Открыть" + ИмяДокумента + "Оформите"], НСтр("ru = 'оформите'"), Истина);
				Иначе
					ОбщееКоличество = ОбщееКоличество + ВывестиПоказатель(
						РезультатЗапроса[ИндексЗапроса].Выбрать(),
						Элементы["Открыть" + ИмяДокумента + "Оформите"], НСтр("ru = 'оформите'"));
				КонецЕсли;
			...
			Если ТекЭлемент.Отработайте Тогда
				ОбщееКоличество = ОбщееКоличество + ВывестиПоказатель(
					РезультатЗапроса[ИндексЗапроса].Выбрать(),
					Элементы["Открыть" + ИмяДокумента + "Отработайте"] , НСтр("ru = 'отработайте'"));
				ИндексЗапроса = ИндексЗапроса + 1;
			КонецЕсли;
			...
			Если ТекЭлемент.Ожидайте Тогда
				ОбщееКоличество = ОбщееКоличество + ВывестиПоказатель(
					РезультатЗапроса[ИндексЗапроса].Выбрать(),
					Элементы["Открыть" + ИмяДокумента + "Ожидайте"],     НСтр("ru = 'ожидайте'"));
				ИндексЗапроса = ИндексЗапроса + 1;
			КонецЕсли;
			
			Если ЕстьПередачаВРегистр2ЕГАИС И ОбщееКоличество = 0 Тогда
				ВывестиПоказатель(
					1,
	...
	Элементы.КорректировкаОстатковЕГАИС.Видимость = ПравоДоступа("Использование", Метаданные.Обработки.ПомощникКорректировкиОстатковЕГАИС);
