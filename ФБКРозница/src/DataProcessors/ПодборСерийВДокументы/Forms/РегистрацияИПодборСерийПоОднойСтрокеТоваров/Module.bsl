...
Процедура СерииНомерПриИзменении(Элемент)
	ПодобратьСерию(Элемент,"Номер","ГоденДо,НомерКиЗГИСМ");
...
Процедура СерииГоденДоПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ГоденДо) Тогда
 		ПодобратьСерию(Элемент,"ГоденДо","Номер,НомерКиЗГИСМ");
	Иначе
		ТекущиеДанные.Серия   = Неопределено;
	КонецЕсли;
...
Процедура ВыполнитьЗапросЗаполненияТаблицыОстатков()
	...
	Если РежимОтображенияСерий = "СерииВДокументе" Тогда
		...
		|	ВЫРАЗИТЬ(СерииВДокументе.Серия КАК Справочник.СерииНоменклатуры).НомерКиЗГИСМ КАК НомерКиЗГИСМ,
		|	ВЫРАЗИТЬ(СерииВДокументе.Серия КАК Справочник.СерииНоменклатуры).RFIDUser КАК RFIDUser,
		|	ВЫРАЗИТЬ(СерииВДокументе.Серия КАК Справочник.СерииНоменклатуры).RFIDEPC КАК RFIDEPC,
		|	ВЫРАЗИТЬ(СерииВДокументе.Серия КАК Справочник.СерииНоменклатуры).EPCGTIN КАК EPCGTIN,
		|	ВЫРАЗИТЬ(СерииВДокументе.Серия КАК Справочник.СерииНоменклатуры).RFIDTID КАК RFIDTID,
		|	ВЫРАЗИТЬ(СерииВДокументе.Серия КАК Справочник.СерииНоменклатуры).RFIDTID <> """" КАК ЗаполненRFIDTID,
		|	ЛОЖЬ КАК НужноЗаписатьМетку,
		|	ВЫБОР
		|		КОГДА ВЫРАЗИТЬ(СерииВДокументе.Серия КАК Справочник.СерииНоменклатуры).RFIDTID <> """"
		|			ТОГДА 2
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК СтатусРаботыRFID
		|ИЗ
	...
	ИначеЕсли РежимОтображенияСерий = "ТолькоОстатки" Тогда
		...
		|	ВЫРАЗИТЬ(ВложенныйЗапрос.Серия КАК Справочник.СерииНоменклатуры).НомерКиЗГИСМ КАК НомерКиЗГИСМ,
		|	ВЫРАЗИТЬ(ВложенныйЗапрос.Серия КАК Справочник.СерииНоменклатуры).RFIDUser КАК RFIDUser,
		|	ВЫРАЗИТЬ(ВложенныйЗапрос.Серия КАК Справочник.СерииНоменклатуры).RFIDEPC КАК RFIDEPC,
		|	ВЫРАЗИТЬ(ВложенныйЗапрос.Серия КАК Справочник.СерииНоменклатуры).EPCGTIN КАК EPCGTIN,
		|	ВЫРАЗИТЬ(ВложенныйЗапрос.Серия КАК Справочник.СерииНоменклатуры).RFIDTID КАК RFIDTID,
		|	ВЫРАЗИТЬ(ВложенныйЗапрос.Серия КАК Справочник.СерииНоменклатуры).RFIDTID <> """" КАК ЗаполненRFIDTID,
		|	ЛОЖЬ КАК НужноЗаписатьМетку,
		|	ВЫБОР
		|		КОГДА ВЫРАЗИТЬ(ВложенныйЗапрос.Серия КАК Справочник.СерииНоменклатуры).RFIDTID <> """"
		|			ТОГДА 2
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК СтатусРаботыRFID
		|ИЗ
...
Процедура ПодобратьСерию(Элемент,ВводимыйРеквизит,ПроверяемыеРеквизиты)
	...
	ИначеЕсли СписокВыбора.Количество() = 2 Тогда
		Если СокрЛП(СписокВыбора[0].Значение.Номер) <> СокрЛП(СписокВыбора[1].Значение.Номер)
			...
			Или СокрЛП(СписокВыбора[0].Значение.НомерКиЗГИСМ) <> СокрЛП(СписокВыбора[1].Значение.НомерКиЗГИСМ)Тогда
