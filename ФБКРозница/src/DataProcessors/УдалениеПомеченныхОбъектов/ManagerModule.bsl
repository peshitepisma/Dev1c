...
Процедура ОтметитьПрогрессОбходаКоллекции(ПараметрыВыполнения, ИмяКоллекции)
	...
	ШагОповещения = Мин(100, Цел(ПараметрыВыполнения.Всего / 10) + 1);
	
	// Проверка что подошло время передачи информации на клиент.
	Если ТекущаяДатаСеанса() < ПараметрыВыполнения.Время
		Или ПараметрыВыполнения.Номер % ШагОповещения > 0 Тогда
		
		Возврат; // Сообщается прогресс о каждых 100 объектах, не чаще, чем раз в 3 секунды.
	КонецЕсли;
