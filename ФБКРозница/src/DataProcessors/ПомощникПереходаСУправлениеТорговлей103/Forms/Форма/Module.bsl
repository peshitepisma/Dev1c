...
Процедура ОбработкаВыбораИБИзСпискаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ПутьКОбработкеСПравилами = Результат;
	
	Если НЕ ЗначениеЗаполнено(ПутьКОбработкеСПравилами) Тогда
...
Процедура ТаблицаПереходовПоСценарию()
	...
	СтруктураПараметров = СоздатьСтруктуруПараметровСтрокиПерехода("СтраницаВыборТипаЗагрузки", 
									"СтраницаНавигацииВыборТипаЗагрузки", "СтраницаДекорацииВыборТипаЗагрукзи");
...
Процедура ПолучитьФайлыОбновления(Знач Оповещение)
	...
	ПолучитьФайлыССервера(ДанныеПользователя);
...
Функция ПолучитьФайлыССервера(ДанныеПользователя)
	...
	ПараметрыПолучения = Новый Структура("ПутьДляСохранения,Пользователь,Пароль");
	...
	Для каждого ЭлемСсылка Из СсылкаНаСкачивание.fileDtoList Цикл
		...
		ПараметрыПолучения.ПутьДляСохранения = ВремКаталог + "\" + ИмяФайла;
		...
		Попытка
			Результат = ПолучениеФайловИзИнтернета.СкачатьФайлНаСервере(Адрес, ПараметрыПолучения, Истина);
