...
Процедура НастроитьПараметрыОтчетаПоВариантуОтчета(НастройкиОтчета, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД)
	...
	Если ПредопределенныйВариант.КлючВарианта = "СебестоимостьТоваровПоПредприятию"
		Или ПредопределенныйВариант.КлючВарианта = "СебестоимостьТоваровПоОперациямПоПредприятию" Тогда
...
Функция ПолучитьПредопределенныйВариант(Знач Вариант)
	...
	КлючиВариантов.Добавить("ДинамикаЗакупок");
...
Функция ТекстЗапросаСебестоимостьТоваровОстаткиИОбороты()
	...
	|	ЕСТЬNULL(СебестоимостьТоваровОстатки.ВидЗапасов.ВладелецТовара, НЕОПРЕДЕЛЕНО) КАК Партнер,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(СебестоимостьТоваровОстатки.АналитикаФинансовогоУчета КАК Справочник.Пользователи), НЕОПРЕДЕЛЕНО) КАК Менеджер,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(СебестоимостьТоваровОстатки.АналитикаФинансовогоУчета КАК Справочник.СтруктураПредприятия), НЕОПРЕДЕЛЕНО) КАК Подразделение,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(СебестоимостьТоваровОстатки.АналитикаФинансовогоУчета КАК Справочник.СделкиСКлиентами), НЕОПРЕДЕЛЕНО) КАК Сделка,
	|	
	|	ВЫБОР 
	...
	|		 (ЕСТЬNULL(ВидЗапасов.ВладелецТовара, НЕОПРЕДЕЛЕНО)).* КАК Партнер,
	|		 (ЕСТЬNULL(ВЫРАЗИТЬ(АналитикаФинансовогоУчета КАК Справочник.Пользователи), НЕОПРЕДЕЛЕНО)).* КАК Менеджер,
	|		 (ЕСТЬNULL(ВЫРАЗИТЬ(АналитикаФинансовогоУчета КАК Справочник.СтруктураПредприятия), НЕОПРЕДЕЛЕНО)).* КАК Подразделение,
	|		 (ЕСТЬNULL(ВЫРАЗИТЬ(АналитикаФинансовогоУчета КАК Справочник.СделкиСКлиентами), НЕОПРЕДЕЛЕНО)).* КАК Сделка
	|		}
...
Функция ТекстЗапросаЗакупкиОбороты()
	...
	|	ЕСТЬNULL(ЗакупкиОбороты.ВидЗапасов.ВладелецТовара, НЕОПРЕДЕЛЕНО) КАК Партнер,
...
Функция ТекстЗапросаВыручкаИСебестоимостьПродажОбороты()
	...
	|	ЕСТЬNULL(ВыручкаИСебестоимостьПродажОбороты.ВидЗапасов.ВладелецТовара, НЕОПРЕДЕЛЕНО) КАК Партнер,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ВыручкаИСебестоимостьПродажОбороты.АналитикаФинансовогоУчета КАК Справочник.Пользователи), НЕОПРЕДЕЛЕНО) КАК Менеджер,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ВыручкаИСебестоимостьПродажОбороты.АналитикаФинансовогоУчета КАК Справочник.СтруктураПредприятия), НЕОПРЕДЕЛЕНО) КАК Подразделение,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ВыручкаИСебестоимостьПродажОбороты.АналитикаФинансовогоУчета КАК Справочник.СделкиСКлиентами), НЕОПРЕДЕЛЕНО) КАК Сделка,
	|	
	|	ВЫБОР 
	|		КОГДА &ФормироватьВидыЗапасовПоГруппамФинансовогоУчета 
	...
	|		 (ВидЗапасов).* КАК ВидЗапасов,
	|		 (ЕСТЬNULL(АналитикаУчетаПартий.Поставщик, НЕОПРЕДЕЛЕНО)).*	КАК Партнер,
	|		 (ЕСТЬNULL(ВЫРАЗИТЬ(АналитикаФинансовогоУчета КАК Справочник.Пользователи), НЕОПРЕДЕЛЕНО)).* КАК Менеджер,
	|		 (ЕСТЬNULL(ВЫРАЗИТЬ(АналитикаФинансовогоУчета КАК Справочник.СтруктураПредприятия), НЕОПРЕДЕЛЕНО)).* КАК Подразделение,
	|		 (ЕСТЬNULL(ВЫРАЗИТЬ(АналитикаФинансовогоУчета КАК Справочник.СделкиСКлиентами), НЕОПРЕДЕЛЕНО)).* КАК Сделка
	|		 (Договор).* КАК Договор,
...
Функция ТекстЗапросаДвиженияНоменклатураНоменклатураОборотыПриход()
	...
	|	ЕСТЬNULL(ДвиженияНоменклатураНоменклатураОборотыПриход.КорВидЗапасов.ВладелецТовара, НЕОПРЕДЕЛЕНО) КАК Партнер,
	|	НЕОПРЕДЕЛЕНО                                                              КАК Менеджер,
	|	НЕОПРЕДЕЛЕНО                                                              КАК Подразделение,
	|	НЕОПРЕДЕЛЕНО                                                              КАК Сделка,
	|	
	...
	|		 (ЕСТЬNULL(КорВидЗапасов.ВладелецТовара, НЕОПРЕДЕЛЕНО)).* КАК Партнер,
...
Функция ТекстЗапросаДвиженияНоменклатураНоменклатураОборотыРасход()
	...
	|	ЕСТЬNULL(ДвиженияНоменклатураНоменклатураОборотыРасход.ВидЗапасов.ВладелецТовара, НЕОПРЕДЕЛЕНО) КАК Партнер,
	|	НЕОПРЕДЕЛЕНО                                                           КАК Менеджер,
	|	НЕОПРЕДЕЛЕНО                                                           КАК Подразделение,
	|	НЕОПРЕДЕЛЕНО                                                           КАК Сделка,
	|	
...
Функция ТекстЗапросаДвиженияНоменклатураДоходыРасходыОбороты()
	...
	|	ЕСТЬNULL(ДвиженияНоменклатураДоходыРасходыОбороты.ВидЗапасов.ВладелецТовара, НЕОПРЕДЕЛЕНО) КАК Партнер,
	|	НЕОПРЕДЕЛЕНО                                                      КАК Менеджер,
	|	НЕОПРЕДЕЛЕНО                                                      КАК Подразделение,
	|	НЕОПРЕДЕЛЕНО                                                      КАК Сделка,
	|	
...
Функция ТекстЗапросаДополнениеВводОстатков()
	...
	|	ЕСТЬNULL(ДополнениеВводОстатков.ВидЗапасов.ВладелецТовара, НЕОПРЕДЕЛЕНО) КАК Партнер,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ДополнениеВводОстатков.АналитикаФинансовогоУчета КАК Справочник.Пользователи), НЕОПРЕДЕЛЕНО) КАК Менеджер,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ДополнениеВводОстатков.АналитикаФинансовогоУчета КАК Справочник.СтруктураПредприятия), НЕОПРЕДЕЛЕНО) КАК Подразделение,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ДополнениеВводОстатков.АналитикаФинансовогоУчета КАК Справочник.СделкиСКлиентами), НЕОПРЕДЕЛЕНО) КАК Сделка,
	|	
	|	ВЫБОР 
	|		КОГДА &ФормироватьВидыЗапасовПоГруппамФинансовогоУчета 
...
Функция ТекстЗапросаСебестоимостьТоваровОборотыПоЗаписям()
	...
	|	ЕСТЬNULL(СебестоимостьТоваровЗаписи.ВидЗапасов.ВладелецТовара, НЕОПРЕДЕЛЕНО) КАК Партнер,
