...
&НаСервере
Функция ПолучитьДанныеПоТоварамУПартнеров()
	
	Запрос = Новый Запрос();
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаСводно.Номенклатура КАК Номенклатура,
	|	ТаблицаСводно.Характеристика КАК Характеристика,
	|	ТаблицаСводно.Серия КАК Серия,
	|	ТаблицаСводно.Назначение КАК Назначение,
	|	СУММА(ТаблицаСводно.Количество) КАК ОстатокТоварыУПартнеров
	|ИЗ
	|	(ВЫБРАТЬ
	|		ОстаткиТоваровУПартнеров.Номенклатура КАК Номенклатура,
	|		ОстаткиТоваровУПартнеров.Характеристика КАК Характеристика,
	|		ОстаткиТоваровУПартнеров.Серия КАК Серия,
	|		ОстаткиТоваровУПартнеров.Назначение КАК Назначение,
	|		ОстаткиТоваровУПартнеров.ПереданоОстаток КАК Количество
	|	ИЗ
	|		РегистрНакопления.ТоварыУПартнеров.Остатки(
	|				,
	|				Договор = &Договор
	|					И Контрагент = &Контрагент
	|					И Партнер = &Партнер) КАК ОстаткиТоваровУПартнеров
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		СторноКорректировки.Номенклатура КАК Номенклатура,
	|		СторноКорректировки.Характеристика КАК Характеристика,
	|		СторноКорректировки.Серия КАК Серия,
	|		СторноКорректировки.Назначение КАК Назначение,
	|		-СторноКорректировки.Передано КАК Количество
	|	ИЗ
	|		РегистрНакопления.ТоварыУПартнеров КАК СторноКорректировки
	|	ГДЕ
	|		СторноКорректировки.Регистратор = &Ссылка) КАК ТаблицаСводно
	|
	|СГРУППИРОВАТЬ ПО
	|	Номенклатура,
	|	Характеристика,
	...
	ПараметрыИзРаспоряжения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ДокументОснование, "Партнер, Контрагент, Договор");
	Для Каждого Параметр Из ПараметрыИзРаспоряжения Цикл
...
&НаСервере
Процедура ОбновитьПризнакЕстьСтрокиКУменьшениюПоТоварамУПартнеров()
	
	ЕстьСтрокиКУменьшениюПоТоварамУПартнеров = Ложь;
	
	Для Каждого Строка Из Объект.Расхождения Цикл
		Если Строка.Количество < 0
			И Не Строка.ПоступилоСколькоПриобрелиПоДвухходовке Тогда
			ЕстьСтрокиКУменьшениюПоТоварамУПартнеров = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	ЕстьСтрокиКУменьшениюПоТоварамУПартнеров = ЕстьСтрокиКУменьшениюПоТоварамУПартнеров
