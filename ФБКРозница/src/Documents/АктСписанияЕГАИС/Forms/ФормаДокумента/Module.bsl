	...
	Иначе
		...
		Если ЗначениеЗаполнено(Результат.Номенклатура)
			...
			Если ТекущиеДанные <> Неопределено
				...
				// Добавление акцизной марки к текущей строке
...
Процедура ОбновитьСтатусЕГАИС()
	...
	РедактированиеФормыНеДоступно = СтатусЕГАИС <> Перечисления.СтатусыОбработкиАктаСписанияЕГАИС.Черновик
	                              И СтатусЕГАИС <> Перечисления.СтатусыОбработкиАктаСписанияЕГАИС.ОшибкаПередачи;
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	ОшибкаПередачи   = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиАктаСписанияЕГАИС.ОшибкаПередачи");
