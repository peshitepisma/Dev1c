...
Процедура ПриИзмененииОрганизацииСервер()
	...
	Объект.АдресДоставкиДляПоставщика = ФормированиеПечатныхФорм.ПолучитьАдресИзКонтактнойИнформации(Объект.Организация, "Фактический", Объект.Дата);
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеДоставкой") Тогда
		ПродажиСервер.ЗаполнитьСписокВыбораАдреса(Элементы.АдресДоставкиДляПоставщика, Объект.Организация,, Объект.Дата);
	Иначе
		ПродажиСервер.ЗаполнитьСписокВыбораАдреса(Элементы.АдресДоставкиДляПоставщика2, Объект.Организация,, Объект.Дата);
	КонецЕсли;
