...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		ЕстьАлкогольнаяПродукция = СтрокаТЧ.АлкогольнаяПродукция > 0;
	КонецЦикла;
	
	Если Не Отказ 
		И ЕстьАлкогольнаяПродукция
		И ИдентификаторыСтрок.Количество() > 0 
		И Объект.АкцизныеМарки.Количество() > 0 Тогда
...
Процедура ПриИзмененииХозяйственнойОперацииСервер()
	...
	НастроитьВидимостьЭлементовЕГАИС();
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов, ЗагрузкаИзТСД = Ложь)
	...
	Если ИспользоватьАкцизныеМарки И ШтрихкодированиеНоменклатурыКлиент.НужноОткрытьФормуУказанияАкцизныхМарокПослеОбработкиШтрихкодов(СтруктураДействий) Тогда
		...
		ДополнительныеПараметрыИзменениеАкцизныхМарок = Новый Структура;
		ДополнительныеПараметрыИзменениеАкцизныхМарок.Вставить("ИдентификаторСтроки", СтруктураДействий.МассивСтрокСАкцизнымиМарками[0]);
		ДополнительныеПараметрыИзменениеАкцизныхМарок.Вставить("Редактирование", Ложь);
...
&НаСервере
Процедура НастроитьВидимостьЭлементовЕГАИС()
	
	ИспользоватьАкцизныеМарки = ИнтеграцияЕГАИСВызовСервера.ИспользуетсяРегистрацияРозничныхПродажВЕГАИС(
		Объект.Организация, Объект.Склад, Объект.Дата) И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтРозничногоПокупателя;
		...
		Если ЗначениеЗаполнено(Результат.Номенклатура)
			...
			Иначе
				...
				Если Объект.Товары.Количество() = 0 Тогда
					...
					КонецЕсли;
				Иначе
					ТекущиеДанные = НайтиСтрокуТЧПоСопоставленнойНоменклатуре(Результат.Номенклатура, Результат.Характеристика, Результат.АлкогольнаяПродукция);
					Если ТекущиеДанные = Неопределено Тогда
						ТекущиеДанные = Объект.Товары.Добавить();
						ЗаполнитьЗначенияСвойств(ТекущиеДанные, Результат, "Номенклатура, Характеристика, Упаковка");
						ТекущиеДанные.НоменклатураЕГАИС = Результат.АлкогольнаяПродукция;
