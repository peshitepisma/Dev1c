...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	ПериодЗаписиЖурнала = 
		?(ЗначениеЗаполнено(Реквизиты.ДатаПолучения) И ВерсияПостановления1137 < 4, Реквизиты.ДатаПолучения, Реквизиты.Период);
...
Функция ТекстЗапросаТаблицаЖурналУчетаСчетовФактур(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И &ПериодЗаписиЖурнала <> ДАТАВРЕМЯ(1,1,1)
	|";
...
Функция ЕстьСчетаФактурыКОформлению(Параметры) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               |	КРегистрации.Дата КАК ДатаСчетаФактуры
	               |ИЗ
