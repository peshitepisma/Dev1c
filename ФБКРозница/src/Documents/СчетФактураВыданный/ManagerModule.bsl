...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	ПериодЗаписиЖурнала = 
		?(ЗначениеЗаполнено(Реквизиты.ДатаВыставления) И ВерсияПостановления1137 < 4, Реквизиты.ДатаВыставления, Реквизиты.Период);  
...
#КонецОбласти
	...
	ТекстЗапроса = ТекстЗапросаВыставленныеСчетаФактурыНаРеализацию
...
Функция ТекстЗапросаДанныхОснованийКОформлениюСчетФактур(Организация = Неопределено)
	...
	Для Каждого ИмяТипа Из МассивТипов Цикл
		...
		Если Не ПравоДоступа("Чтение", Метаданные.Документы[ИмяТипа]) Тогда
			Продолжить;
		КонецЕсли;
		ТекстЗапросаДляТипа = ШаблонЗапроса;
		Если ПервыйЗапрос Тогда
			ТекстЗапросаДляТипа = СтрЗаменить(ТекстЗапросаДляТипа, "ВЫБРАТЬ", "ВЫБРАТЬ РАЗРЕШЕННЫЕ");
			ПервыйЗапрос = Ложь;
		КонецЕсли;
		ТекстЗапросаДляТипа = СтрЗаменить(ТекстЗапросаДляТипа, "%ИмяДокумента%", ИмяТипа);
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		УсловиеОтбора = "ТребуетсяОформлениеСчетаФактуры.Организация = &Организация";
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&УсловиеОтбораОрганизация", УсловиеОтбора);
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&УсловиеОтбораОрганизация", "ИСТИНА");
	КонецЕсли;
...
Функция ЕстьСчетаФактурыКОформлению(Организация = Неопределено) Экспорт
		
	ТекстЗапроса = ТекстЗапросаДанныхОснованийКОформлениюСчетФактур(Организация);
...
Функция СформироватьГиперссылкуКОформлению(Параметры) Экспорт
	...
	Если ПоказыватьСчетаФактуры И ЕстьСчетаФактурыКОформлению(Организация) Тогда
