...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ПодключаемоеОборудование" И ВводДоступен() Тогда
		Если ИмяСобытия = "ScanData" И МенеджерОборудованияКлиентПереопределяемый.ЕстьНеобработанноеСобытие() Тогда
			...
			ПараметрыСканированияАкцизныхМарок.КонтрольАкцизныхМарок = ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЧекККМ");
			ПараметрыСканированияАкцизныхМарок.ИмяКолонкиКоличество = "КоличествоФакт";
			
			ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
			Если ТекущиеДанные <> Неопределено Тогда
				ПараметрыСканированияАкцизныхМарок.ИдентификаторСтроки = ТекущиеДанные.ПолучитьИдентификатор();
			КонецЕсли;
			...
			АкцизныеМаркиЕГАИСКлиент.ОбработатьДанныеШтрихкода(ОповещенияПриЗавершении, ЭтотОбъект, ДанныеШтрихкодов[0], ПараметрыСканированияАкцизныхМарок);
...
Процедура АкцизныеМарки(Команда)
	...
	ДополнительныеПараметры.КонтрольАкцизныхМарок     = ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЧекККМ");
	...
	ДополнительныеПараметры.ИмяКолонкиКоличество      = "КоличествоФакт";
...
Процедура Подключаемый_ПолученаАкцизнаяМарка(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Редактирование Тогда
		...
		ТекущиеДанные = Товары.НайтиПоИдентификатору(ДополнительныеПараметры.ИдентификаторСтроки);
	...
	Иначе
		...
		Иначе
			НайденнаяСтрока = Товары.НайтиПоИдентификатору(ДополнительныеПараметры.ИдентификаторСтроки);
		...
		Если ЗначениеЗаполнено(Результат.Номенклатура)
			...
			Иначе
				...
				Если Товары.Количество() = 0 Тогда
					ТекущиеДанные = Товары.Добавить();
					ЗаполнитьЗначенияСвойств(ТекущиеДанные, Результат, "Номенклатура, Характеристика, Упаковка");
					ТекущиеДанные.НоменклатураЕГАИС = Результат.АлкогольнаяПродукция;
					...
					КонецЕсли;
				Иначе
					ИдентификаторСтроки = НайтиСтрокуТЧПоСопоставленнойНоменклатуре(Результат.Номенклатура, Результат.Характеристика, Результат.АлкогольнаяПродукция);
...
Процедура ИзменениеАкцизныхМарокЗавершение(ТекущиеДанные, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("ОткрытьПодборСерий", ТекущиеДанныеИдентификатор) Тогда
...
&НаСервере
Функция НайтиСтрокуТЧПоСопоставленнойНоменклатуре(Номенклатура, Характеристика, АлкогольнаяПродукция)
	...
	Для Каждого СтрокаТЧ Из НайденныеСтроки Цикл
		...
		Если Результат = Неопределено Тогда
			Результат = СтрокаТЧ.ПолучитьИдентификатор();
		Иначе
			Если СтрокаТЧ.НоменклатураЕГАИС = АлкогольнаяПродукция Тогда
				Результат = СтрокаТЧ.ПолучитьИдентификатор();
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	Если ИспользоватьАкцизныеМарки И ШтрихкодированиеНоменклатурыКлиент.НужноОткрытьФормуУказанияАкцизныхМарокПослеОбработкиШтрихкодов(СтруктураДействий) Тогда
		...
		Если НужноОткрытьФормуУказанияСерийПослеОбработкиШтрихкодов Тогда
			ДополнительныеПараметрыИзменениеАкцизныхМарок.Вставить("ОткрытьПодборСерий", СтруктураДействий.МассивСтрокССериями[0]);
		КонецЕсли;
