...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	ИначеЕсли Настройки["ОрганизацияЕГАИС"] = Неопределено Тогда
		...
		МассивОрганизаций = ИнтеграцияЕГАИСКлиентСервер.ОрганизацииЕГАИС(НастройкиОбмена.ОбменНаСервере);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивОрганизаций, ИнтеграцияЕГАИСКлиентСервер.ОрганизацииЕГАИС(НастройкиОбмена.ОбменНаКлиенте));
		
		Если МассивОрганизаций.Количество() = 1 Тогда
			ОрганизацияЕГАИС = МассивОрганизаций[0];
		КонецЕсли;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВходящееСообщениеОперация = Сообщения[Выборка.ИсходящееСообщение].ОжидаемаяОперация Тогда
			...
			Если РезультатОбработки.Количество() > 0 Тогда
				Объект = ИнтеграцияЕГАИСВызовСервера.ОбработатьВходящиеДокументы(ДокументыКЗагрузке).Изменения[0].Объект;
			Иначе
				Объект = Неопределено;
			КонецЕсли;
