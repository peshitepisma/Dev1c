...
Функция ЗаписатьСвернутыеДвиженияПоРезервамТоваровОрганизаций(СтруктураДанныхСвертки)
	...
	Пока ВыборкаРегистраторы.Следующий() Цикл
		...
		|ПОМЕСТИТЬ ВТКорректирующиеНаборыПолей
		|ИЗ
		|	(ВЫБРАТЬ
		|		Т.Регистратор КАК Регистратор,
		|		Т.НомерНабораПолей КАК НомерНабораПолей
		...
		|				ИЛИ Т.РегистраторСторно = &Регистратор)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ГДЕ
		|		Т.Регистратор = &Регистратор
		|		И Т.РегистраторСторно <> НЕОПРЕДЕЛЕНО) КАК Т
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	НомерНабораПолей,
		|	Регистратор
		...
		|	Поля.СлужебноеВидДвиженияПриход,
		|	Поля.АналитикаУчетаНоменклатуры,
		|	Поля.Организация,
		|	Поля.ВидЗапасов,
		|	Поля.НомерГТД,
		|	Поля.КорОрганизация,
		...
		|	Движения.НомерНабораПолей,
		|	Движения.РегистраторСторно
		|ПОМЕСТИТЬ ВТКорректирующиеДвижения
		|ИЗ
		|	ВТТаблицаДвижений КАК Движения
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКорректирующиеНаборыПолей КАК ЗаписываемыеНаборыПолей
		|		ПО Движения.НомерНабораПолей = ЗаписываемыеНаборыПолей.НомерНабораПолей
		|			И Движения.Регистратор = ЗаписываемыеНаборыПолей.Регистратор
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТНаборыПолей КАК Поля
		...
		|ВЫБРАТЬ
		|	Т.Регистратор КАК Регистратор,
		|	МИНИМУМ(ВЫБОР КОГДА Т.Количество > 0
		|		ТОГДА 0
		|		ИНАЧЕ 1
		|	КОНЕЦ) КАК ПриоритетЗаписи
		|ПОМЕСТИТЬ ВТРегистраторы
		|ИЗ
		|	ВТКорректирующиеДвижения КАК Т
		|
		|СГРУППИРОВАТЬ ПО
		|	Т.Регистратор
		|
