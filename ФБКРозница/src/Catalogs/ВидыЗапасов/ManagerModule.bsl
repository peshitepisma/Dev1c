...
Функция ТекстЗапросаПоискаДублейВидовЗапасов(ЭтоОбработчикОбновления = Ложь) Экспорт
...
Процедура УстановитьПараметрыЗапросаПоискаДублей(Запрос, МассивОрганизаций = Неопределено, ПроверятьПометкуУдаления = Ложь) Экспорт
...
Процедура ЗаменитьДублиВидовЗапасов() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СоответствиеЗаписей.Вставить(Выборка.Дубль, Выборка.КорректныйВидЗапасов);
		
		Если НЕ Выборка.ПометкаУдаленияДубля Тогда
			УдалитьДублирующийЭлемент(Выборка.Дубль);
		КонецЕсли;
...
Процедура ПометитьДублиВидовЗапасов() Экспорт
	...
	|	ВидыЗапасов.ПометкаУдаления КАК ПометкаУдаления,
	|	ВидыЗапасов.ЭтоДубль КАК ЭтоДубль,
	|	Отборы.КоличествоДублей КАК КоличествоДублей
	|ИЗ
	|	СпрВидыЗапасов КАК ВидыЗапасов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтОтборыДублей КАК Отборы
	|		ПО ВидыЗапасов.Организация = Отборы.Организация
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЭтоПервыйПроход Тогда
			ЗаполнитьЗначенияСвойств(СтруктураДляСравнения, Выборка);
			ЭтоПервыйПроход = Ложь;
			ОбработатьСсылки(Выборка, ЭтоПервоеСовпадение, МассивСсылок);
			Продолжить;
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(СтруктураИзВыборки, Выборка);
		Если ОбщегоНазначенияУТКлиентСервер.СтруктурыРавны(СтруктураДляСравнения, СтруктураИзВыборки) Тогда
			ОбработатьСсылки(Выборка, ЭтоПервоеСовпадение, МассивСсылок);
		Иначе
			
			// заменим найденные ссылки...
			ЗаменитьСсылки(МассивСсылок);
						
			// ... и перейдем к следующему элементу
			ЗаполнитьЗначенияСвойств(СтруктураДляСравнения, Выборка);
...
Процедура ЗаменитьСсылки(МассивСсылок)
	
	НайденныеСсылки = НайтиПоСсылкам(МассивСсылок);
	
	ТаблицаСсылок = Новый ТаблицаЗначений;
	ТаблицаСсылок.Колонки.Добавить("ВидЗапасов", Новый ОписаниеТипов("СправочникСсылка.ВидыЗапасов"));
	ТаблицаСсылок.Колонки.Добавить("КоличествоСсылок", Новый ОписаниеТипов("Число"));
	
	ТаблицаСсылок.ЗагрузитьКолонку(МассивСсылок, "ВидЗапасов");
	
	Для Каждого НайденнаяСсылка Из НайденныеСсылки Цикл
		НоваяСтрока = ТаблицаСсылок.Добавить();
		НоваяСтрока.ВидЗапасов = НайденнаяСсылка.Ссылка;
	КонецЦикла;
	
	ТаблицаСсылок.ЗаполнитьЗначения(1, "КоличествоСсылок");
	ТаблицаСсылок.Свернуть("ВидЗапасов", "КоличествоСсылок");
	
	Сравнение = Новый СравнениеЗначений;
	ТаблицаСсылок.Сортировать("КоличествоСсылок Убыв", Сравнение);
	
	Счетчик = 1;
	КоличествоЭлементов = ТаблицаСсылок.Количество();
	Пока Счетчик <= КоличествоЭлементов - 1 Цикл
		НачатьТранзакцию();
		Попытка
			// первый элемент оставляем как есть - по нему больше всего ссылок,
			// остальные помечаем как дубли.
			СпрОбъект = ТаблицаСсылок[Счетчик].ВидЗапасов.ПолучитьОбъект();
		...
		Исключение
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Ссылка%", ТаблицаСсылок[Счетчик].ВидЗапасов);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Справочники.ВидыЗапасов,
				ТаблицаСсылок[Счетчик].ВидЗапасов,
				ТекстСообщения);
		КонецПопытки;
		Счетчик = Счетчик + 1;
	КонецЦикла;
...
Процедура ОбработатьСсылки(Выборка, ЭтоПервоеСовпадение, МассивСсылок)
	
	Если Выборка.КоличествоДублей = 2 Тогда
		Если ЭтоПервоеСовпадение Тогда
			// поля упорядочены по признаку ПометкаУдаления,
			// первую запись пропускаем. Вторую, помеченную на удаление,
			// помечаем как дубль.
			ЭтоПервоеСовпадение = Ложь; 
		Иначе
			НачатьТранзакцию();
			Попытка
				СпрОбъект = Выборка.Ссылка.ПолучитьОбъект();
				СпрОбъект.ЭтоДубль = Истина;
				ОбработатьРеквизитыОбъекта(СпрОбъект);
				СпрОбъект.ДополнительныеСвойства.Вставить("НеПроверятьДубли", Истина);
			...
			КонецПопытки;
		КонецЕсли;
	Иначе // дублей > 2, надо искать количество ссылок на каждый элемент
