...
Процедура СформироватьДанныеСчетовФактур(СчетаФактуры, ВременныеТаблицы) 
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МИНИМУМ(ВтСчетаФактуры.Дата) КАК Дата
		...
		Если Не Результат[2].Пустой() Тогда
			...
			Пока ВыборкаПоДокументам.Следующий() Цикл
				Если ВыборкаПоДокументам.СуммаНДС <> 0 Тогда
					...
					Исключение
						ОтменитьТранзакцию();
						ОбновлениеИнформационнойБазыУТ.СообщитьОНеудачнойОбработке(ВыборкаПоДокументам.Ссылка);
						ВызватьИсключение;
					КонецПопытки;
				Иначе
					ТаблицаДанных.Добавить().СчетФактура = ВыборкаПоДокументам.Ссылка;
			...
			Исключение
				ОтменитьТранзакцию();
				ОбновлениеИнформационнойБазыУТ.СообщитьОНеудачнойОбработке(Документы.ТаможеннаяДекларацияИмпорт.ПустаяСсылка());
				ВызватьИсключение;
			КонецПопытки;
		КонецЕсли;
#КонецОбласти
