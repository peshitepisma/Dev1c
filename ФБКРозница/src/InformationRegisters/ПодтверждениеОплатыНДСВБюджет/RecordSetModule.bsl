...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Если Отбор.Состояние.Значение <> Перечисления.СостоянияОплатыНДСВБюджет.ОжидаетОплаты Тогда
...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Пока Выборка.Следующий() Цикл
		Если Не ЗначениеЗаполнено(Выборка.Период) ИЛИ Выборка.Организация = Null Тогда
			...
			Продолжить;
		КонецЕсли;
		ПроверитьДатуЗапрета(Выборка.Период);
		РегистрыСведений.ЗаданияКФормированиюЗаписейКнигиПокупокПродаж.СоздатьЗаписьРегистра(Выборка.Период,
