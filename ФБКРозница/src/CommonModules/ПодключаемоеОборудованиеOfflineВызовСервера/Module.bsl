...
//  ДокументСсылка.ОтчетОРозничныхПродажах - Загруженный отчет о розничных продажах.
//
...
//  * МассивКодовSKU - Массив - массив кодов SKU.
//
Функция ПолучитьДанныеДляКассы(Устройство, ТолькоИзмененные = Истина, РасширеннаяВыгрузка = Ложь) Экспорт
	...
	ВозвращаемоеЗначение.Вставить("МассивКодовSKU", Таблица.ВыгрузитьКолонку("Код"));
	
	Если РасширеннаяВыгрузка Тогда
		
		СтруктураПрайсЛиста = МенеджерОборудованияСервисыКлиентСервер.ПолучитьСтруктуруПрайсЛиста();
		ЗаполнитьСтруктуруПрайсЛистаИзДанныхКВыгрузке(СтруктураПрайсЛиста, Таблица);
		СтруктураПрайсЛиста.ПолнаяЗагрузка = НЕ (ТолькоИзмененные И Параметры.ВыгружатьИзменения);
		
		ВозвращаемоеЗначение.Данные = СтруктураПрайсЛиста;
	...
	Иначе
		...
		Для Каждого СтрокаТЧ Из Таблица Цикл
			...
			Иначе
				...
				ЭлементМассива = Новый Структура("КодВРегистре, PLU, Код, Штрихкод, Наименование, НаименованиеПолное, ЕдиницаИзмерения, Цена, Остаток, ВесовойТовар, Артикул", 0, 0, 0, "", "", "", "", 0, 0, Ложь, "");
				ЭлементМассива.Код          = Число(СтрокаТЧ.Код);
				ЭлементМассива.PLU          = Число(СтрокаТЧ.Код);
				ЭлементМассива.КодВРегистре = Число(СтрокаТЧ.Код);
				Если СтрокаТЧ.Используется Тогда
...
Процедура ЗаполнитьСтруктуруПрайсЛистаИзДанныхКВыгрузке(СтруктураПрайсЛиста, ТоварыКВыгрузке) Экспорт
	...
	|	ТоварыКВыгруке.ЕдиницаИзмеренияНаименование КАК ЕдиницаИзмеренияНаименование,
	...
	|	ТоварыКВыгруке.КодНалога КАК КодНалога,
	...
	|	Товары.КодНалога КАК КодНалога,
	...
	Пока ВыборкаПоНоменклатуре.Следующий() Цикл
		...
		ЗаписьТовар.Услуга           = ВыборкаПоНоменклатуре.ТипНоменклатуры = ТипНоменклатурыУслуга;
		ЗаписьТовар.КодНалога        = ВыборкаПоНоменклатуре.КодНалога;
		ЗаписьТовар.Алкоголь         = ВыборкаПоНоменклатуре.Алкоголь;
		ЗаписьТовар.Код              = ВыборкаПоНоменклатуре.Код;
...
Функция ПолучитьТаблицуТоваровКВыгрузке(Устройство, Параметры, ОбновитьКодыТоваров = Ложь) Экспорт
	...
	|	
	|	ВЫБОР
	|		КОГДА КодыТоваровПодключаемогоОборудованияOffline.Номенклатура.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18)
	|			ТОГДА 1
	|		ИНАЧЕ ВЫБОР
	|				КОГДА КодыТоваровПодключаемогоОборудованияOffline.Номенклатура.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС10)
	|					ТОГДА 2
	|				ИНАЧЕ 3
	|			КОНЕЦ
	|	КОНЕЦ КАК КодНалога,
	...
	Пока ВыборкаПоКодам.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(НоваяСтрока.Код) Тогда
				...
				НоваяСтрока.КодНалога                   = Выборка.КодНалога;
