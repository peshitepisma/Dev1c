...
Функция КатегорииОпераций() Экспорт
	...
	ПередачаДанных.Вставить(
		...
		НСтр("ru = 'Передача сведений о розничной продаже маркируемой алкольной продукции'"));
	ПередачаДанных.Вставить(
	...
	ПередачаДанных.Вставить(
		...
		НСтр("ru = 'Подверждение акта расхождений ТТН'"));
	...
	ПередачаДанных.Вставить(
		...
		НСтр("ru = 'Передача сведений о передаче алкогольной продукции в регистра №2'"));
	...
	ПолучениеДанных.Вставить(
		Перечисления.ВидыДокументовЕГАИС.КвитанцияАктаРасхождений,
		НСтр("ru = 'Получение квитанции подверждения акта расхождений или отказа от акта расхождений'"));
	ПолучениеДанных.Вставить(
		Перечисления.ВидыДокументовЕГАИС.КвитанцияАктаРасхожденийОтказ,
		НСтр("ru = 'Получение квитанции отказа от акта расхождений ТТН'"));
	ПолучениеДанных.Вставить(
		Перечисления.ВидыДокументовЕГАИС.КвитанцияАктаРасхожденийПодтверждение,
		НСтр("ru = 'Получение квитанции подверждения акта расхождений ТТН'"));
...
//   * ДальнейшиеДействия - Массив - Дальнейшие действия при операци.
//
...
Функция ПредыдущаяОперация(ПоследовательностьОпераций, СтрокаПоследовательности, Знач ИсходныйИндекс = Неопределено) Экспорт
	...
	Если ПредыдущаяОперация.Операция = Перечисления.ВидыДокументовЕГАИС.АктТТН
		...
		Или ПредыдущаяОперация.Операция = Перечисления.ВидыДокументовЕГАИС.КвитанцияЗапросаНаОтменуПроведенияТТН Тогда
...
Процедура РассчитатьСтатусОформления(МетаданныеДокумента, ДокументОснование, РеквизитыДокументаОснования, ЗаписьТребуется) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Если (Не РеквизитыДокументаОснования.Проведен Или Не ЗаписьТребуется) Тогда
			...
			РегистрыСведений.СтатусыОформленияДокументовЕГАИС.УдалитьЗаписьРаспряженияНаОформление(ДокументОснование, ПустаяСсылка);
			Возврат;
...
Функция СоздатьСправку(ДанныеСправки, ВидСправки) Экспорт
	...
	ИмяСправочника = ?(ВидСправки = Перечисления.ВидыДокументовЕГАИС.ОтветНаЗапросСправки1, "Справки1ЕГАИС", "Справки2ЕГАИС");
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		СправкаОбъект = Выборка.Ссылка.ПолучитьОбъект();
	Иначе
		СправкаОбъект = Справочники[ИмяСправочника].СоздатьЭлемент();
	КонецЕсли;
	
	Если ВидСправки = Перечисления.ВидыДокументовЕГАИС.ОтветНаЗапросСправки2 Тогда
...
//
Процедура ОбновитьФорматОбменаОрганизацииЕГАИС(КодФСРАР, ФорматОбмена) Экспорт
...
Процедура ЗаполнитьСопоставленнуюПродукцию(ТаблицаСопоставления) Экспорт
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаСопоставления КАК ТаблицаСопоставления
...
Процедура ПриИзмененииСтатусаДокумента(ДокументСсылка, ПредыдущийСтатус, НовыйСтатус, ОбновлятьДвижения = Истина) Экспорт
	...
	ИнтеграцияЕГАИСПереопределяемый.ПриИзмененииСтатусаДокумента(ДокументСсылка, ПредыдущийСтатус, НовыйСтатус);
...
//
Процедура ЗаполнитьВXDTOАкцизныеМарки(ОбъектXDTO, Источник, ИсточникАкцизныеМарки, СообщениеXML) Экспорт
	
	КодыАкцизныхМарок = Новый Массив;
	НайденныеСтроки = ИсточникАкцизныеМарки.НайтиСтроки(Новый Структура("ИдентификаторСтроки", Источник.ИдентификаторСтроки));
	...
	Если КодыАкцизныхМарок.Количество() > 0 Тогда
		...
		Для Каждого КодАкцизнойМарки Из КодыАкцизныхМарок Цикл
			ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(ОбъектXDTO.MarkCodeInfo, "MarkCode", КодАкцизнойМарки, СообщениеXML);
		КонецЦикла;
...
Функция НайтиОбъектПоИдентификатору(Метаданные, ИмяРеквизита, Значение, ВидКвитанции = Неопределено) Экспорт
	...
	Если ИмяРеквизита = "ИдентификаторЗапроса" Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ " + Метаданные.ПолноеИмя() + " КАК ТаблицаДокументы
	...
	ИначеЕсли ВидКвитанции = Неопределено Тогда
		...
		|ИЗ
		|	" + Метаданные.ПолноеИмя() + " КАК Таблица
		|ГДЕ
...
Функция ИсходящееСообщение(ИдентификаторЗапроса) Экспорт
	...
	|ГДЕ
	|	ПротоколОбмена.ИдентификаторЗапроса = &ИдентификаторЗапроса
	|	И ПротоколОбмена.ТипСообщения = ЗНАЧЕНИЕ(Перечисление.ТипыЗапросовЕГАИС.Входящий)");
...
Функция ЗагрузитьТТНВходящуюЕГАИС(ДанныеДокументаТТН, ДанныеДокументаУведомлениеОРегистрации, ОрганизацияЕГАИС) Экспорт
	...
	Реквизиты.Вставить("Документ",             Ссылка);
	Реквизиты.Вставить("СообщениеОснование",   Неопределено);
	Реквизиты.Вставить("Описание",             НСтр("ru = 'Получено уведомление о регистрации двиежения ТТН ЕГАИС (входящая)'"));
	...
	Если РезультатДобавленияЗаписиТТН.НовоеСообщение Тогда
		...
		ГрузоотправительXDTO = ДанныеОрганизации(ДанныеДокументаТТН.Объект.Header.Shipper);
		ОбновитьФорматОбменаОрганизацииЕГАИС(ГрузоотправительXDTO.ClientRegId, ДанныеДокументаТТН.ФорматОбмена);
		...
		Для Каждого ЭлементДанных Из ДанныеДокументаТТН.Объект.Content.Position Цикл
			Если АлкогольнаяПродукция.Найти(ЭлементДанных.Product) = Неопределено Тогда
		...
		Для Каждого ЭлементДанных Из ДанныеДокументаТТН.Объект.Content.Position Цикл
			...
			НоваяСтрока.АлкогольнаяПродукция  = СоответствиеЗагруженнаяАлкогольнаяПродукция[ЭлементДанных.Product.AlcCode];
			...
			Если ДанныеДокументаТТН.ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V1 Тогда
				MarkInfo = ЭлементДанных.InformB.InformBItem.MarkInfo;
			...
			Иначе
				MarkInfo = ЭлементДанных.InformF2.InformF2Item.MarkInfo;
			...
			Если MarkInfo <> Неопределено Тогда
				...
				Для Каждого ДанныеДиапазона Из MarkInfo.Ranges.Range Цикл
					
					Диапазон = Новый Структура;
					Диапазон.Вставить("ТипМарки",       MarkInfo.Type);
		...
		НовыйСтатус = МенеджерОбъекта.ОбновитьСтатусПослеПолученияДанных(
			...
			ПараметрыОбновленияСтатуса(Ложь));
		
		ИнтеграцияЕГАИСПереопределяемый.ПриЗагрузкеТТН(ДокументОбъект);
...
Функция ЗагрузитьУведомлениеОРегистрацииДвиженияТТН(ДанныеДокумента, ОрганизацияЕГАИС, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РезультатДобавленияЗаписи.НовоеСообщение Тогда
		...
		Для Каждого ЭлементДанных Из ДанныеДокумента.Объект.Content.Position Цикл
			...
			СтрокаТЧ = ДокументОбъект.Товары[НомерСтроки - 1];
		...
		НовыйСтатус = МенеджерОбъекта.ОбновитьСтатусПослеПолученияДанных(
			...
			ПараметрыОбновленияСтатуса(Не ЗаписыватьДокумент));
...
Функция ЗагрузитьУведомлениеОРегистрацииДвиженияАктаПостановкиНаБаланс(ДанныеДокумента, ОрганизацияЕГАИС, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РезультатДобавленияЗаписи.НовоеСообщение Тогда
		...
		Для Каждого СтрокаКвитанцииXDTO Из ДанныеДокумента.Объект.Content.Position Цикл
			...
			Если ДокументОбъект.Товары.Количество() > ИндексСтроки Тогда
				
				СтрокаТЧ = ДокументОбъект.Товары[ИндексСтроки];
				
				ДанныеСправки1.АлкогольнаяПродукция    = СтрокаТЧ.АлкогольнаяПродукция;
				ДанныеСправки1.НомерТТН                = СтрокаТЧ.НомерТТН;
				ДанныеСправки1.ДатаТТН                 = СтрокаТЧ.ДатаТТН;
				ДанныеСправки1.ДатаРозлива             = СтрокаТЧ.ДатаРозлива;
				ДанныеСправки1.Количество              = СтрокаТЧ.КоличествоПоСправке1;
				ДанныеСправки1.НомерПодтвержденияЕГАИС = СтрокаТЧ.НомерПодтвержденияЕГАИС;
				ДанныеСправки1.ДатаПодтвержденияЕГАИС  = СтрокаТЧ.ДатаПодтвержденияЕГАИС;
...
//
Функция ЗагрузитьАлкогольнуюПродукцию(СписокАлкогольнойПродукцииXDTO) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ИнтеграцияЕГАИСПереопределяемый.ПриЗагрузкеАлкогольнойПродукции(АлкогольнаяПродукцияОбъект);
		АлкогольнаяПродукцияОбъект.Записать();
		
		Результат.Вставить(Выборка.Код, АлкогольнаяПродукцияОбъект.Ссылка);
	...
	Если РезультатДобавленияЗаписи.НовоеСообщение Тогда
		...
		Если ДанныеДокумента.Объект.Products <> Неопределено
			...
			Для Каждого ЭлементДанных Из ДанныеДокумента.Объект.Products.StockPosition Цикл
				...
				СтрокаТЧ.АлкогольнаяПродукция = СоответствиеЗагруженнаяАлкогольнаяПродукция[ЭлементДанных.Product.AlcCode];
...
Функция ЗагрузитьОтветНаЗапросОтчетаДвиженияМеждуРегистрами(ДанныеДокумента, ОрганизацияЕГАИС, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РезультатДобавленияЗаписи.НовоеСообщение Тогда
		...
		Если ЕстьОшибки Тогда
			НовыйСтатус = МенеджерОбъекта.ОбновитьСтатусПослеПолученияДанных(
				ДокументОбъект.Ссылка, ДанныеДокумента.Операция,
				ПараметрыОбновленияСтатуса(Ложь,,Перечисления.СтатусыОбработкиСообщенийЕГАИС.Ошибка));
		КонецЕсли;
	...
	Если РезультатДобавленияЗаписи.НовоеСообщение Тогда
		...
		Если ОтразитьФиксацию И ЗначениеЗаполнено(ДанныеОснованияКвитанции.Ссылка) Тогда
			...
			НовыйСтатус = МенеджерОбъекта.ОбновитьСтатусПослеПолученияДанных(
				...
				ДанныеДокумента.Операция,
				ПараметрыОбновленияСтатуса(Не ЗаписыватьДокумент, ДанныеОснованияКвитанции.Операция, СтатусОбработки));
...
Функция ЗагрузитьКвитанциюПроведенЕГАИС(ДанныеДокумента, ДанныеОснованияКвитанции, ИдентификаторЕГАИС, ОрганизацияЕГАИС) Экспорт
	...
	Реквизиты.Вставить("Операция",             ДанныеДокумента.Операция);
	Реквизиты.Вставить("Документ",             ДанныеОснованияКвитанции.Ссылка);
	Реквизиты.Вставить("СообщениеОснование",   ДанныеОснованияКвитанции.ИсходящееСообщение);
...
Функция ДанныеСправки(ДанныеДокумента, ВидСправки)
	...
	ДанныеСправки.АлкогольнаяПродукция = СоответствиеЗагруженнаяАлкогольнаяПродукция[ДанныеДокумента.Объект.Product.AlcCode];
...
// Обработчик подписки на событие "При записи" для документов, влиящий на расчет статуса Акта постановки на баланс ЕГАИС.
//
// Параметры:
...
// Обработчик подписки на событие "При записи" для документов, влиящий на расчет статуса Акта списания ЕГАИС.
//
// Параметры:
//  Источник - ДокументОбъект - источник события.
//  Отказ    - см. описание в синтаксис помощнике к обработчику события объекта ПриЗаписи.
...
// Обработчик подписки на событие "При записи" для документов, влиящий на расчет статуса Возврат из регистра №2 ЕГАИС.
//
// Параметры:
//  Источник - ДокументОбъект - источник события.
//  Отказ    - см. описание в синтаксис помощнике к обработчику события объекта ПриЗаписи.
...
// Обработчик подписки на событие "При записи" для документов, влиящий на расчет статуса Передача в регистр №2 ЕГАИС.
//
// Параметры:
//  Источник - ДокументОбъект - источник события.
//  Отказ    - см. описание в синтаксис помощнике к обработчику события объекта ПриЗаписи.
...
// Обработчик подписки на событие "При записи" для документов, влиящий на расчет статуса ТТН ЕГАИС (исходящая).
//
// Параметры:
//  Источник - ДокументОбъект - источник события.
//  Отказ    - см. описание в синтаксис помощнике к обработчику события объекта ПриЗаписи.
...
// Обработчик подписки на событие "При записи" для документов, влиящий на расчет статуса ТТН ЕГАИС (входящая).
//
// Параметры:
//  Источник - ДокументОбъект - источник события.
//  Отказ    - см. описание в синтаксис помощнике к обработчику события объекта ПриЗаписи.
...
// Обработчик подписки на событие "При записи" для документов, влиящий на расчет статуса Чек ЕГАИС.
//
// Параметры:
//  Источник - ДокументОбъект - источник события.
//  Отказ    - см. описание в синтаксис помощнике к обработчику события объекта ПриЗаписи.
...
// Обработчик подписки на событие "При записи" для документов, влиящий на расчет статуса Чек ЕГАИС на возврат.
//
// Параметры:
//  Источник - ДокументОбъект - источник события.
//  Отказ    - см. описание в синтаксис помощнике к обработчику события объекта ПриЗаписи.
...
Процедура ОтборПоОрганизацииПриСозданииНаСервере(Форма, Знач ЗначениеПрефиксы = Неопределено) Экспорт
	...
	Для Каждого Значение Из Префиксы Цикл
		Форма.Элементы[Значение + "ОрганизацияЕГАИС"].СписокВыбора.Очистить();
		Форма.Элементы[Значение + "ОрганизацииЕГАИС"].СписокВыбора.Очистить();
	КонецЦикла;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Значение <> Неопределено Тогда
			...
			Для Каждого Значение Из Префиксы Цикл
				Форма.Элементы[Значение + "ОрганизацияЕГАИС"].СписокВыбора.Добавить(Данные, СтрСоединить(Данные, "; "));
				Форма.Элементы[Значение + "ОрганизацииЕГАИС"].СписокВыбора.Добавить(Данные, СтрСоединить(Данные, "; "));
			КонецЦикла;
...
// Возвращает стуктуру параметров обновления статуса.
//
// Параметры:
...
//   * СтатусОбработки - ПеречислениеСсылка.СтатусыОбработкиСообщенийЕГАИС - Статус обработки сообщения.
//
Функция ПараметрыОбновленияСтатуса(ОбновлятьДвижения = Ложь, ОперацияКвитанции = Неопределено, СтатусОбработки = Неопределено) Экспорт
	
	ПараметрыОбновленияСтатуса = Новый Структура;
	ПараметрыОбновленияСтатуса.Вставить("ОбновлятьДвижения", ОбновлятьДвижения);
	ПараметрыОбновленияСтатуса.Вставить("ОперацияКвитанции", ОперацияКвитанции);
	ПараметрыОбновленияСтатуса.Вставить("СтатусОбработки",   СтатусОбработки);
...
//
// Параметры:
//  ЗаписьНабора - ЗаписьНабора - Напись набора.
...
Функция ПараметрыОбновленияПослеПолученияДанных(ЗаписьНабора, МенеджерОбъекта, ДокументСсылка, Операция,
	...
	Если ЭтоКвитанция Тогда
		...
		ДополнительныеПараметры.Вставить("ОперацияКвитанции", ОперацияКвитанции);
		ДополнительныеПараметры.Вставить("СтатусОбработки",   СтатусОбработки);
...
Функция ПодготовитьДанныеДляПодбораСправок2(Товары)
	...
	ТаблицаДляСписания.Свернуть("АлкогольнаяПродукция, Номенклатура, Характеристика, Справка2", "Количество");
	ТаблицаДляЗаполнения.Свернуть("АлкогольнаяПродукция, Номенклатура, Характеристика", "Количество");
	...
	ВозвращаемоеЗначение.Вставить("ТаблицаДляСписания",                 Результат[Результат.Количество() - 1].Выгрузить());
	ВозвращаемоеЗначение.Вставить("ТаблицаДляЗаполнения",               Результат[Результат.Количество() - 2].Выгрузить());
	ВозвращаемоеЗначение.Вставить("КоэффициентыПересчетаВЕдиницыЕГАИС", Результат[Результат.Количество() - 3].Выгрузить());
...
//
Функция ЗаполнитьСправку2ВТабличнойЧасти(Товары, АлкогольнаяПродукция, Справка2, ОстатокПоСправке, СтруктураПересчетаСуммы, КоэффициентыПересчетаВЕдиницыЕГАИС)
	...
	Пока ОстатокПоСправке > 0 Цикл
		...
		Для Каждого СтрокаТЧ Из МассивСтрок Цикл
			...
			НайденныеСтроки = КоэффициентыПересчетаВЕдиницыЕГАИС.НайтиСтроки(ПараметрыОтбора);
			Если НайденныеСтроки.Количество() > 0 Тогда
				СтрокаТаблицыКоэффициентов = НайденныеСтроки[0];
			Иначе
			...
			ОстатокПоСправкеПриведенный = Цел(ОстатокПоСправке / СтрокаТаблицыКоэффициентов.Коэффициент);
		...
		Если КоличествоНеподходящихСтрокТЧ = МассивСтрок.Количество() Тогда
			Прервать;
		КонецЕсли;
