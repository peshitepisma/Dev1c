		...
		Для Каждого ЭлементОчереди Из Очередь Цикл
			...
			Попытка
				...
				Иначе
					...
					Если СтрНайти(ВРег(ДанныеДокумента.АдресЗапроса), ВРег("TTNHistoryF2Reg")) = 0 Тогда
...
//   * ДокументОснование - ДокументСсылка - основание документа, по которому было получено или отрправлено сообщение.
...
Функция ПроверитьСопоставлениеКлассификаторов(ДокументОснование, УникальныйИдентификатор = Неопределено) Экспорт
	...
	|	И Товары.Номенклатура = &ПустаяНоменклатура
	|");
...
Функция ПроверитьРасхожденияТоваровПоступленияТТН_ЕГАИС(ДокументСсылка) Экспорт
	...
	Возврат ИнтеграцияЕГАИСПереопределяемый.ПроверитьРасхожденияТоваровПоступленияТТН_ЕГАИС(ДокументСсылка);
...
//
// Параметры:
//  ДокументСсылка - ДокументСсылка.ТТНВходящаяЕГАИС - подтверждаемая ТТН.
//  Расхождения - ТаблицаЗначений, Неопределено - таблица с фактически принятым количеством.
//
Процедура ЗаполнитьРасхожденияВДокументеТТНВходящая(ДокументСсылка, Расхождения = Неопределено) Экспорт
	...
	Попытка
		...
		ДокументОбъект.ЕстьРасхождения = Расхождения <> Неопределено;
