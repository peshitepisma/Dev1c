...
Функция ТекстЗапросаПубликуемыхТоваров(ФормироватьСвойства = Ложь, ВключатьТолькоСопоставленные = Истина,
	...
	|	ТорговыеПредложения.ВидЦены КАК СрокПоставки
	...
	|	СоглашенияСКлиентамиТовары.ВидЦены.ВалютаЦены КАК Валюта,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	КурсыВалюты.Валюта КАК Валюта,
	|	КурсыВалюты.Курс КАК Курс,
	|	КурсыВалюты.Кратность КАК Кратность
	|ПОМЕСТИТЬ КурсыВалют
	|ИЗ
	|	РегистрСведений.КурсыВалют.СрезПоследних({(&Период)}, ) КАК КурсыВалюты
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Валюта
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	РегУчетнаяПолитика.УчетнаяПолитика.СистемаНалогообложения КАК СистемаНалогообложения
	|ПОМЕСТИТЬ РегУчетнаяПолитика
	|ИЗ
	...
	|					ТорговыеПредложения.Организация КАК Организация
	|				ИЗ
	|					ТорговыеПредложения КАК ТорговыеПредложения)) КАК РегУчетнаяПолитика
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|					И РегУчетнаяПолитика.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Общая)
	...
	|		ПО (СоглашениеТовары.Ссылка В (&ТорговоеПредложение))
	...
	|		ПО (СоглашениеШапка.Ссылка В (&ТорговоеПредложение))
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегУчетнаяПолитика КАК РегУчетнаяПолитика
	|		ПО (ИСТИНА)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	УточненияНоменклатуры.ТорговоеПредложение КАК Организация
	...
	|			ТОГДА Товар.Ссылка.ЕдиницаИзмерения
	|		ИНАЧЕ Товар.Упаковка
	|	КОНЕЦ КАК Упаковка,
	|	Товар.Валюта.Код КАК ВалютаКод,
	|	Товар.Характеристика КАК Характеристика,
	|	Товар.Ссылка.Наименование КАК Наименование,
	|	Товар.Ссылка.Артикул КАК Артикул,
	|	СоответствиеВидовНоменклатуры1СБизнесСеть.ИдентификаторКатегории КАК ИдентификаторКатегории,
	...
	|	Товар.Ссылка.СтавкаНДС КАК СтавкаНДС,
	|	Товар.ТорговоеПредложение КАК ТорговоеПредложение,
	|	Товар.Организация КАК Организация,
	|	Товар.ВидЦены КАК ВидЦены
	|ПОМЕСТИТЬ ВыборкаНоменклатуры
	|ИЗ
	|	НоменклатураПоСегменту КАК Товар
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеВидовНоменклатуры1СБизнесСеть КАК СоответствиеВидовНоменклатуры1СБизнесСеть
	|		ПО Товар.Ссылка.ВидНоменклатуры = СоответствиеВидовНоменклатуры1СБизнесСеть.ВидНоменклатуры
	...
	|			{(&Период)},
	|			Цена > 0
	|				И ЗНАЧЕНИЕ(Справочник.СегментыНоменклатуры.ПустаяСсылка) В
	|					(ВЫБРАТЬ
	|						ТорговыеПредложения.СегментНоменклатуры КАК СегментНоменклатуры
	|					ИЗ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТорговыеПредложения КАК ТорговыеПредложения
	|		ПО (ТорговыеПредложения.ВидЦены = ЦеныНоменклатурыСрезПоследних.ВидЦены)
	|ГДЕ
	|	ВыборкаУточненийНоменклатуры.Номенклатура ЕСТЬ NULL
	|	И ЗНАЧЕНИЕ(Справочник.СегментыНоменклатуры.ПустаяСсылка) В
	|			(ВЫБРАТЬ
	...
	|	ВыборкаУточненийНоменклатуры.Организация,
	...
	|	ВыборкаНоменклатуры.Организация КАК Организация,
	...
	|					ИЗ
	|						ВыборкаНоменклатуры КАК ВыборкаНоменклатуры)) КАК СвободныеОстаткиОстатки
	...
	|	ВыборкаНоменклатуры.Организация,
	...
	Если Не ИспользоватьПараметры Тогда
		ТекстТовары = СтрЗаменить(ТекстТовары, "СоглашениеТовары.Ссылка В (&ТорговоеПредложение)", "ИСТИНА");
		ТекстТовары = СтрЗаменить(ТекстТовары, "СоглашениеШапка.Ссылка В (&ТорговоеПредложение)", "ИСТИНА");
