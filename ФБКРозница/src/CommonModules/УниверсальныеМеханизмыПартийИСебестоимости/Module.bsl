...
Функция ДокументыСРазнымиПериодамиИлиОрганизациямиВДвижениях(РазныеПериоды, РазныеОрганизации, ИмяРегистра = "") Экспорт
	...
	Если ИмяРегистра = ""
	 ИЛИ ИмяРегистра = Метаданные.РегистрыНакопления.ПрочиеРасходы.Имя Тогда
		...
		Если РазныеПериоды Тогда
			ОписаниеДокументов.Вставить(Метаданные.Документы.РаспределениеРасходовБудущихПериодов, НЕ Значение);
		КонецЕсли;
...
Процедура ПроверитьКорректностьОстатковВРегистреСебестоимостьТоваровПослеРасчета(ПараметрыРасчета)
	...
	Если НЕ ЗначениеЗаполнено(Проверка) Тогда
		...
		ТекстДляПротокола = НСтр("ru='Не найдена проверка ""СоответствиеСуммИКоличества"".'");
...
Функция ОбработатьИсключениеВызоваРассчитатьВсе(ПараметрыРасчета, ИнформацияОбОшибке, Отказ) Экспорт
	...
	// Если надо - передадим информацию об ошибку вышестоящей Попытка-Исключение
	ВызыватьИсключение = НЕ ПараметрыРасчета.ИдетРасчетПартий;
	
	ОкончаниеРасчета(ПараметрыРасчета); // запишем протокол
	
	Если ВызыватьИсключение Тогда
		// Для ошибки расчета себестоимости расчет надо прервать в Попытке-Исключение "верхнего уровня" в модуле ПартионныйУчет22.
...
Процедура ПроверкаКорректностиИсходныхДанныхДляРасчетаСебестоимости(ПараметрыПроверки) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЕстьВозвратныеОтходы КАК Отходы
...
Функция ОписаниеПараметровОперацииЗакрытияМесяца() Экспорт
	...
	#Область РешениеСЛУ
		...
		ОписаниеПараметра.ЗначениеПоУмолчанию = 31;
		ОписаниеПараметра.ДиапазонС     	  = 23;
		ОписаниеПараметра.ДиапазонПо     	  = 38;
		ОписаниеПараметра.Описание =
			НСтр("ru='Максимальная разрядность чисел, допустимая в используемой СУБД (38 - для всех СУБД кроме DB2, для DB2 - 31).
		...
		ОписаниеПараметра.ДиапазонПо     	  = 38;
		ОписаниеПараметра.Описание =
			НСтр("ru='Максимально допустимая разрядность числа 1*10^х - значения суммы в строке временной таблицы решений при решении СЛУ.
	...
	#Область Проверки
		...
		ОписаниеПараметра.Описание =
			...
			|Проверка пока по умолчанию отключена, т.к. еще находится в режиме тестирования.'");
