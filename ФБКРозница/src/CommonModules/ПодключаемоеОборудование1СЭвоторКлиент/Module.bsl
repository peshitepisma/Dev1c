...
Функция ВыполнитьКоманду(Команда, ВходныеПараметры = Неопределено, ВыходныеПараметры = Неопределено,
	...
	ИначеЕсли Команда = "ЗагрузитьОтчет" ИЛИ Команда = "ЗагрузитьРасширенныйОтчет" Тогда
		Результат = ЗагрузитьДокументы(Параметры, ВходныеПараметры, ВыходныеПараметры, Команда);
	ИначеЕсли Команда = "ВыгрузитьТовары" Тогда 
		Результат = ПередатьТовары(Параметры, ВходныеПараметры[0], ВыходныеПараметры, Команда);
...
Процедура НачатьВыполнениеКоманды(ОповещениеПриЗавершении, Команда, ВходныеПараметры = Неопределено, ОбъектДрайвера, Параметры, ПараметрыПодключения) Экспорт
	...
	ИначеЕсли Команда = "ВыгрузитьТовары" Тогда
		НачатьПередачуТоваров(ОповещениеПриЗавершении, Параметры, ВходныеПараметры[0], ВыходныеПараметры, Команда);
	ИначеЕсли Команда = "ОчиститьБазу" Тогда
...
Функция ЗагрузитьМагазины(Параметры, ВходныеПараметры, ВыходныеПараметры, Команда)
	...
	Результат = ПодключаемоеОборудование1СЭвоторВызовСервера.ОтправитьЗапрос(ПараметрыЗапроса, ВыходныеПараметры);
	Если Результат Тогда
		РезультатЗапроса = ВыходныеПараметры[0];
		СтруктураОтвета = ПодключаемоеОборудование1СЭвоторВызовСервера.ЗаполнитьСтруктуруОтвета(РезультатЗапроса, ВыходныеПараметры);
	КонецЕсли;
...
Функция ЗагрузитьДокументы(Параметры, ВходныеПараметры, ВыходныеПараметры, Команда)
	...
	Если Результат Тогда
		...
		Если ЗначениеЗаполнено(РезультатЗапроса) Тогда
			РезультатОбработки = ПодключаемоеОборудование1СЭвоторВызовСервера.ЗаполнитьСтруктуруОтвета(РезультатЗапроса, ДанныеОтвета);
		Иначе
			РезультатОбработки = Ложь;
		КонецЕсли;
		
		Если РезультатОбработки Тогда
			ВыходныеПараметры = ПодключаемоеОборудование1СЭвоторВызовСервера.ОбработатьДанныеЗапроса(ДанныеОтвета, Команда);
		КонецЕсли;
	КонецЕсли;
...
Функция ПередатьТовары(Параметры, ВходныеПараметры, ВыходныеПараметры, Команда)
	...
	ТекстЗапроса = ПодключаемоеОборудование1СЭвоторВызовСервера.ПолучитьТекстJSONЗапроса(ВходныеПараметры, "ВыгрузитьТовары");
...
Функция УдалитьТовары(Параметры, ВходныеПараметры, ВыходныеПараметры, Команда)
	...
	ТекстЗапроса = ПодключаемоеОборудование1СЭвоторВызовСервера.ПолучитьТекстJSONЗапроса(ВходныеПараметры, "УдалитьТовары");
...
// Функция осуществляет загрузку терминало из облака Эвотор
//
...
Процедура НачатьЗагрузкуДокументов(ОповещениеПриЗавершении, Параметры, ВходныеПараметры, ВыходныеПараметры, Команда)
	...
	ДополнительныеПараметры.Вставить("ПараметрыЗапроса",        ПараметрыЗапроса);
	ДополнительныеПараметры.Вставить("НаименованиеКоманды",     Команда);
	ДополнительныеПараметры.Вставить("НачалоВыгрузки",          Параметры.ПериодНачалоВыгрузки);
...
Процедура НачатьОтправкуДанных(Результат, ДополнительныеПараметры) Экспорт
	...
	ВыходныеПараметры = Новый Массив;
	ТекстЗапроса = ПодключаемоеОборудование1СЭвоторВызовСервера.ПолучитьТекстJSONЗапроса(ДополнительныеПараметры.ВходныеПараметры, ДополнительныеПараметры.НаименованиеКоманды);
	ДополнительныеПараметры.ПараметрыЗапроса.Вставить("ТекстЗапроса", ТекстЗапроса);
	
	Результат = ПодключаемоеОборудование1СЭвоторВызовСервера.ОтправитьЗапрос(ДополнительныеПараметры.ПараметрыЗапроса, ВыходныеПараметры);
...
Процедура НачатьПринятиеДанных(Результат, ДополнительныеПараметры) Экспорт
	...
	Результат = ПодключаемоеОборудование1СЭвоторВызовСервера.ОтправитьЗапрос(ДополнительныеПараметры.ПараметрыЗапроса, ТелоЗапроса);
	Если Результат Тогда
		СтруктураОтвета = ПодключаемоеОборудование1СЭвоторВызовСервера.ЗаполнитьСтруктуруОтвета(ТелоЗапроса[0], ВыходныеПараметры);
	Иначе
...
Процедура НачатьОбработкуДанных(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		КонецЕсли;
		СтруктураДанных = ПодключаемоеОборудование1СЭвоторВызовСервера.ОбработатьДанныеЗапроса(Результат, ПараметрыКоманды);
...
Функция ЗаполнитьПараметрыЗапроса(КомандаЗапроса, Параметры)
	...
	ИначеЕсли КомандаЗапроса = "ЗагрузитьТовары" Тогда
		...
		МетодЗапроса = "GET";
	ИначеЕсли КомандаЗапроса = "ЗагрузитьОтчет" ИЛИ КомандаЗапроса = "ЗагрузитьРасширенныйОтчет" Тогда
		...
		НачалоВыгрузки = ПодключаемоеОборудование1СЭвоторВызовСервера.ПреобразоватьДату(Параметры.ПериодВыгрузки.ДатаНачала - 86400);
		...
		ОкончаниеВыгрузки = ПодключаемоеОборудование1СЭвоторВызовСервера.ПреобразоватьДату(Параметры.ПериодВыгрузки.ДатаОкончания);
	...
	ИначеЕсли КомандаЗапроса = "ВыгрузитьТовары" Тогда
		АдресЗапроса = "api/v1/inventories/stores/" + Параметры.МагазинЗначение + "/products";
		МетодЗапроса = "POST";
