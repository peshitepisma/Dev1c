...
//
Процедура ПроверитьКорректностьЗаполненияАлкогольнойПродукции(ДанныеДляЕГАИС, Объект, Отказ) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДублиАкцизныхМарок.КодАкцизнойМарки КАК КодАкцизнойМарки,
	...
	|		ПО (АкцизныеМарки.КодАкцизнойМарки = ДублиАкцизныхМарок.КодАкцизнойМарки)
	|ИТОГИ ПО
	|	КодАкцизнойМарки");
	Запрос.УстановитьПараметр("АкцизныеМарки", Объект.АкцизныеМарки.Выгрузить());
	ВыборкаКодАкцизнойМарки = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаКодАкцизнойМарки.Следующий() Цикл
		
		НомерСтроки = Неопределено;
		НомераСтрок = Новый Массив;
		ВыборкаИдентификаторыСтрок = ВыборкаКодАкцизнойМарки.Выбрать();
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(
				НСтр("ru='Акцизная марка %1 дублируется в строках %2 табличной части ""Товары"".'"),
				ВыборкаКодАкцизнойМарки.КодАкцизнойМарки,
	...
	Для Каждого СтрокаТЧ Из ДанныеДляЕГАИС Цикл
		
		Если Не СтрокаТЧ.АлкогольнаяПродукция Тогда
			Продолжить;
		КонецЕсли;
		
		Если СтрокаТЧ.АлкогольнаяПродукцияВоВскрытойТаре Тогда
			Продолжить;
		КонецЕсли;
		
		Если Не РазрешатьПродажуАлкогольнойПродукцииБезСопоставленияЕГАИС Тогда
			
			Если Не ЗначениеЗаполнено(СтрокаТЧ.НоменклатураЕГАИС) Тогда
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Объем) Тогда
		...
		Если СтрокаТЧ.МаркируемаяАлкогольнаяПродукция Тогда
			...
			Если ТипЗнч(СтрокаТЧ.КодАкцизнойМарки) = Тип("Строка")
				И ЗначениеЗаполнено(СтрокаТЧ.КодАкцизнойМарки) Тогда
				КоличествоАкцизныхМарок = 1;
			КонецЕсли;
			Если ТипЗнч(СтрокаТЧ.КодАкцизнойМарки) = Тип("Массив") Тогда
				КоличествоАкцизныхМарок = СтрокаТЧ.КодАкцизнойМарки.Количество();
			КонецЕсли;
			
			Если СтрокаТЧ.Количество <> КоличествоАкцизныхМарок Тогда
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(
						НСтр("ru='Не корректно заполнены акцизные марки для алкогольной продукции %1 в строке %2 списка ""Товары"".'"),
						СтрокаТЧ.Номенклатура,
						СтрокаТЧ.НомерСтроки),
					Объект,
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(СтрокаТЧ.Крепость) Тогда
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(
						НСтр("ru='Не заполнена крепость для алкогольной продукции %1 в строке %2 списка ""Товары"".'"),
						СтрокаТЧ.Номенклатура,
						СтрокаТЧ.НомерСтроки),
					Объект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", СтрокаТЧ.НомерСтроки, "Номенклатура"),
			...
			Если Не ЗначениеЗаполнено(СтрокаТЧ.ПроизводительИмпортерАлкогольнойПродукции) Тогда
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(
						НСтр("ru='Не заполнен производитель (импортер) для алкогольной продукции %1 в строке %2 списка ""Товары"".'"),
						СтрокаТЧ.Номенклатура,
						СтрокаТЧ.НомерСтроки),
					Объект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", СтрокаТЧ.НомерСтроки, "Номенклатура"),
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(СтрокаТЧ.ИНН) Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтрШаблон(
							НСтр("ru='Не заполнено ИНН производителя (импортера) для алкогольной продукции %1 в строке %2 списка ""Товары"".'"),
							СтрокаТЧ.Номенклатура,
							СтрокаТЧ.НомерСтроки),
						Объект,
						ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", СтрокаТЧ.НомерСтроки, "Номенклатура"),
				...
				Если Не ЗначениеЗаполнено(СтрокаТЧ.КПП) Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						СтрШаблон(
							НСтр("ru='Не заполнено КПП производителя (импортера) для алкогольной продукции %1 в строке %2 списка ""Товары"".'"),
							СтрокаТЧ.Номенклатура,
							СтрокаТЧ.НомерСтроки),
						Объект,
						ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", СтрокаТЧ.НомерСтроки, "Номенклатура"),
			...
			Если Не ЗначениеЗаполнено(СтрокаТЧ.ВидПродукции) Тогда
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(
						НСтр("ru='Не заполнен вид продукции для алкогольной продукции %1 в строке %2 списка ""Товары"".'"),
						СтрокаТЧ.Номенклатура,
						СтрокаТЧ.НомерСтроки),
					Объект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", СтрокаТЧ.НомерСтроки, "Номенклатура"),
...
//
Процедура СопоставитьАлкогольнуюПродукциюСНоменклатурой(Объект, ИмяКолонки = "НоменклатураЕГАИС") Экспорт
	
	Данные = Объект.Товары.Выгрузить(, ИмяКолонки + ", " + "Номенклатура, Характеристика");
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Т.%ИмяКолонки% КАК АлкогольнаяПродукция,
	|	Т.Номенклатура,
	|	Т.Характеристика
	|ПОМЕСТИТЬ Таблица
	|ИЗ
