...
//
Процедура ПриЗагрузкеАлкогольнойПродукции(АлкогольнаяПродукция) Экспорт
...
Функция ТаблицаРасхожденийТТН_ЕГАИС(ДокументСсылка) Экспорт
	...
	|		ПО ТабличнаяЧасть.Ссылка = ТТНВходящаяЕГАИС.ДокументОснование
	|		
...
Функция ПроверитьРасхожденияТоваровПоступленияТТН_ЕГАИС(ДокументСсылка) Экспорт
	...
	ВозвращаемоеЗначение.Вставить("ЕстьРасхожденияБольшеТТН", Ложь);
	ВозвращаемоеЗначение.Вставить("ЕстьРасхожденияМеньшеТТН", Ложь);
	...
	ВозвращаемоеЗначение.ЕстьРасхожденияБольшеТТН = Не Результат[2].Пустой();
	ВозвращаемоеЗначение.ЕстьРасхожденияМеньшеТТН = Не Результат[3].Пустой();
	//-- НЕ ЕГАИС
...
Функция ПолучитьКоличествоВозможныхДокументовПоступленияТоваров(Организация, ТорговыйОбъект, Контрагент) Экспорт
	...
	|ГДЕ
	|	ПриобретениеТоваровУслуг.Организация = &Организация
...
Процедура ОбработкаЗаполненияДокумента(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПередачаВРегистр2ЕГАИС") Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	Сопоставлено.Номенклатура           КАК Номенклатура,
			|	Сопоставлено.Характеристика         КАК Характеристика,
			|	ТабличнаяЧасть.АлкогольнаяПродукция КАК АлкогольнаяПродукция,
