...
Процедура ПодготовитьКПередаче(ДокументСсылка, ДальнейшееДействие) Экспорт
	...
	Если ДальнейшееДействие = ПредопределенноеЗначение("Перечисление.ДальнейшиеДействияПоВзаимодействиюЕГАИС.ПодтвердитеПолучение") Тогда
		
		ПодтвердитьПолучение(ДокументСсылка);
...
Процедура ПодготовитьСообщенияКПередачеЗавершение(Контекст) Экспорт
	...
	Если Контекст.ДальнейшееДействие = ПредопределенноеЗначение("Перечисление.ДальнейшиеДействияПоВзаимодействиюЕГАИС.ПодтвердитеПолучение") Тогда
		
		Если Контекст.МассивДокументов.Количество() > 1 Тогда
			ПоказатьПредупреждение(
				Неопределено,
				НСтр("ru = 'Команда может быть выполнена только для одного объекта.'"));
			Возврат;
		КонецЕсли;
		
		ПодтвердитьПолучение(Контекст.МассивДокументов[0]);
...
// Выполняет подготовку запроса данных классификатов ЕГАИС к передаче и
...
Процедура ПроверитьСопоставлениеКлассификаторов(ДокументОснование, Форма, ОповещениеПриЗавершении) Экспорт
	...
	Иначе
		...
		РезультатПроверки = ИнтеграцияЕГАИСВызовСервера.ПроверитьСопоставлениеКлассификаторовПрикладногоДокумента(
			ДокументОснование,
			Форма.УникальныйИдентификатор);
		
		ИмяФормы = "Обработка.СопоставлениеКлассификаторовЕГАИС.Форма.СопоставлениеНоменклатурыСАлкогольнойПродукциейЕГАИС";
...
Процедура ПодтвердитьПолучение(ДокументСсылка)
	
	СтруктураРасхождений = ИнтеграцияЕГАИСВызовСервера.ПроверитьРасхожденияТоваровПоступленияТТН_ЕГАИС(ДокументСсылка);
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ТоварноТранспортнаяНакладнаяЕГАИС", ДокументСсылка);
	ДополнительныеПараметры.Вставить("ЕстьРасхожденияБольшеТТН",          СтруктураРасхождений.ЕстьРасхожденияБольшеТТН);
	...
	Если СтруктураРасхождений.ЕстьРасхожденияБольшеТТН ИЛИ СтруктураРасхождений.ЕстьРасхожденияМеньшеТТН Тогда
		...
		ПараметрыВопроса = Новый Структура("ТоварноТранспортнаяНакладнаяЕГАИС", ДокументСсылка);
		ОткрытьФорму(
			"Документ.ТТНВходящаяЕГАИС.Форма.ФормаВопросОРасхожденияхЕГАИС",
			ПараметрыВопроса,,,,,
			Новый ОписаниеОповещения(
				"ПодготовитьСообщенияКПередачеПослеВопросаОРасхождениях",
				ИнтеграцияЕГАИССлужебныйКлиент,
				ДополнительныеПараметры));
