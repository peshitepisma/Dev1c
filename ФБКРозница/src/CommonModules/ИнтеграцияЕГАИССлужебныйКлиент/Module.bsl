...
// Только для внутреннего использования.
// Вызывается из: ПодтвердитьПолучение
Процедура ПодготовитьСообщенияКПередачеПослеВопросаОРасхождениях(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Результат.ПринятьСРасхождениями Тогда
		
		ИнтеграцияЕГАИСВызовСервера.ЗаполнитьРасхожденияВДокументеТТНВходящая(
			ДополнительныеПараметры.ТоварноТранспортнаяНакладнаяЕГАИС,
			Результат.ТаблицаРасхождений);
...
Процедура ПослеУдаленияЗагруженногоДокумента(РезультатОтправкиHTTPЗапроса, Контекст) Экспорт
	...
	Если Не РезультатОперации.ТекстСообщенияXMLПолучен Тогда
		...
		//				СтрШаблон(
		//					НСтр("ru = 'Не удалось удалить входящий документ %1 (%2)
		//					           |%3'"),
		//					Данные.Операция,
		//					Данные.АдресЗапроса,
		//					РезультатОперации.ТекстОшибки));
