...
Функция ПредставлениеСчетаФактуры(Номер, Дата, Проведен) Экспорт
	...
	Возврат НСтр("ru='Счет-фактура'") + " " +"№ " + СокрЛП(Номер) + " " + НСтр("ru='от'") + " " + Формат(Дата, "ДЛФ=D") + " " + НСтр("ru='г.'") + " " + ?(НЕ Проведен, НСтр("ru='(не проведен)'"),"");
...
Функция ПредставлениеСчетаФактурыВДокументеЗакупки(Основание, Организация, НеТребуется = Ложь) Экспорт
	...
	Уточнение = ?(ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаРеализации"), НСтр("ru='полученный'") + " ","");
	...
	Если СчетаФактуры.Количество() > 0 Тогда
		...
		Иначе
			...
			СтрокаГиперссылки = Новый ФорматированнаяСтрока(
				ПредставлениеСчетаФактуры(РеквизитыСчетаФактуры.Номер, РеквизитыСчетаФактуры.ДатаСоставления,РеквизитыСчетаФактуры.Проведен), ,
				ЦветаСтиля.ЦветГиперссылки, ,
