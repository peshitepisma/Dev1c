...
Функция ТекстДополненияПартийПрочих() // использует РегистраторыПрочих
	Возврат "
		...
		|		И ДД.Организация В (&МассивОрганизаций)
		|		И ДД.Активность
		|	ИЛИ Статьи.ВариантРаспределенияРасходовУпр <>
		|		ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров)
		|	ИЛИ Статьи.ВариантРаспределенияРасходовРегл <>
		|			ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров)
		|		ИЛИ (Статьи.Ссылка ЕСТЬ NULL
		|			И НалогообложениеНДС <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОпределяетсяРаспределением))
		|	)
		|";
КонецФункции
...
Функция ТекстБазисПриходы() // использует АналитикиРасходовВсе
	Возврат "
		...
		|	СУММА(ВЫРАЗИТЬ(&Вес КАК ЧИСЛО(15,3))
		|		* ВЫБОР КОГДА ДД.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|			ТОГДА ДД.Количество
		|		ИНАЧЕ -ДД.Количество КОНЕЦ) КАК Вес,
		|	СУММА(ВЫРАЗИТЬ(&Объем КАК ЧИСЛО(15,3))
		|		* ВЫБОР КОГДА ДД.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|			ТОГДА ДД.Количество
...
Функция ВходящиеДанныеМеханизма(ВходящиеДанные = Неопределено, ТолькоТребующиеПерерасчета = Ложь) Экспорт
	...
	Если НЕ ТолькоТребующиеПерерасчета Тогда
		...
		ВходящиеДанные.Вставить(Метаданные.Документы.СчетФактураВыданный, Значение);
		ВходящиеДанные.Вставить(Метаданные.Документы.СчетФактураПолученный, Значение);
