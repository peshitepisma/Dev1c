...
//
Функция ПравилаЗаполненияПоляТипЗаписи() Экспорт
	...
	Для НомерСтроки = 2 По Макет.ВысотаТаблицы Цикл
		...
		// 6. Указание кор. партии в расходе
		КорПартияВРасходе = СокрЛП(Макет.Область(НомерСтроки, 6, НомерСтроки, 6).Текст);
		НовоеПравило.КорПартияВРасходе = НЕ ПустаяСтрока(КорПартияВРасходе);
		
		// 7. Комментарий (не используется)
