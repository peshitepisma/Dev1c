...
Процедура СформироватьНачальныеОстаткиНДСПредъявленный(ПараметрыРасчета)
	...
	|ИЗ
	|	РегистрНакопления.НДСПредъявленный.Остатки(&ГраницаКонецПредыдущегоПериода,	Организация В (&МассивОрганизаций) И &ВалютыУпрИРеглУчетаСовпадают) КАК Т
	|ГДЕ
	|	Т.НДСОстаток <> 0
	|
	|ИНДЕКСИРОВАТЬ ПО
...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета)
	...
	|				И НЕ Т.КорОрганизация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	...
	#Область УпрСуммыВДвижениях
	
	// Проверка соответствия заполненности суммы упр. и ФО "ВестиУправленческийУчетОрганизаций".
	РасшифровкаКодовОшибок = Новый Соответствие;
...
Функция ТекстЗаказыИзАналитикРасходов() // ВТ ЗаказыИзАналитикРасходов, ПоступленияИзАналитикРасходов, ПоступленияИЗаказыИзАналитикРасходов, ОстаткиПоступленийПоЗаказам 
	Возврат
		...
		|			ПО ДД.ЗаказПоставщику = Товары.Ссылка
...
Функция ТекстАналитикиПрошлыхПоступленийТоваров() // вт АналитикиПрошлыхПоступлений
...
Функция ТекстБазисПрошлыеПриходыТоваровСебестоимость()
	...
	ТекстЗапроса = 
		...
		|	НЕОПРЕДЕЛЕНО КАК АналитикаРаспределения,
		|	0 КАК Базис,
		|	СУММА(ДД.Количество - ЕСТЬNULL(ПрошлыеВыбытияТоваров.Количество, 0)) КАК Количество,
		|	СУММА(ВЫРАЗИТЬ(&Вес КАК ЧИСЛО(15,3)) * ДД.Количество - ВЫРАЗИТЬ(&Вес КАК ЧИСЛО(15,3)) * ЕСТЬNULL(ПрошлыеВыбытияТоваров.Количество, 0)) КАК Вес,
		|	СУММА(ВЫРАЗИТЬ(&Объем КАК ЧИСЛО(15,3)) * ДД.Количество - ВЫРАЗИТЬ(&Объем КАК ЧИСЛО(15,3)) * ЕСТЬNULL(ПрошлыеВыбытияТоваров.Количество, 0)) КАК Объем,
		|	СУММА(ДД.Стоимость - ЕСТЬNULL(ПрошлыеВыбытияТоваров.Стоимость, 0)) КАК Стоимость,
		|	СУММА(ДД.СтоимостьБезНДС - ЕСТЬNULL(ПрошлыеВыбытияТоваров.СтоимостьБезНДС, 0)) КАК СтоимостьБезНДС,
		|	СУММА(ДД.СтоимостьРегл - ЕСТЬNULL(ПрошлыеВыбытияТоваров.СтоимостьРегл, 0)) КАК СтоимостьРегл,
		|	0 КАК НДСРегл,
		|	0 КАК НДСУпр,
		|	СУММА(ДД.ПостояннаяРазница - ЕСТЬNULL(ПрошлыеВыбытияТоваров.ПостояннаяРазница, 0)) КАК ПостояннаяРазница,
		|	СУММА(ДД.ВременнаяРазница - ЕСТЬNULL(ПрошлыеВыбытияТоваров.ВременнаяРазница, 0)) КАК ВременнаяРазница,
		|	ДД.ВидДеятельностиНДС КАК ВидДеятельностиНДС,
		|	ДД.ВидДеятельностиНДС КАК НалогообложениеНДС,
		|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка) КАК ХозяйственнаяОперация,
		|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КАК ПодразделениеРасходов,
		...
		|		И ПрошлыеВыбытияТоваров.РазделУчета = ДД.РазделУчета
		|		И ПрошлыеВыбытияТоваров.ВидЗапасов = ДД.ВидЗапасов
		|		И ПрошлыеВыбытияТоваров.Организация = ДД.Организация
		|		И ПрошлыеВыбытияТоваров.Партия = ДД.Партия
		|		И ПрошлыеВыбытияТоваров.АналитикаУчетаПартий = ДД.АналитикаУчетаПартий
...
Функция ОписаниеЦепочекПартийНДС(ПараметрыРасчета)
	...
	// Дополнение, ОтклонениеВСтоимости, РаспределениеПоВыбывшимТоварам <- (<без источников - для формирования записей по данным запроса>)
	ДобавитьОписаниеДополнения(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.Дополнение);
...
Процедура ОптимизироватьНумерациюВЦепочках(ПараметрыРасчета)
	...
	|	СУММА(ВЫБОР КОГДА СвязиУзлов.СвязанныйУзел ЕСТЬ NULL ТОГДА 0 ИНАЧЕ 1 КОНЕЦ) КАК КоличествоСвязей,
	|	СУММА(ВЫБОР КОГДА СвязиУзлов.СвязанныйУзел ЕСТЬ NULL ТОГДА 0 ИНАЧЕ 1 КОНЕЦ) КАК ЗначениеРазделителя
...
Процедура РассчитатьПартииПоЦепочкам(ПараметрыРасчета)
	...
	Если МногопоточныйРасчет Тогда
		УниверсальныеМеханизмыПартийИСебестоимости.УничтожитьВременныеТаблицы(ПараметрыРасчета,
			"ОписаниеПодграфовДляРасчетаНеВФоне");
	КонецЕсли;
...
Функция ТестРасчетныеПартии(ИмяМетода, НачалоПериода, КонецПериода, Организация, ДанныеДляПартий, ВнутренниеДанные = Неопределено) Экспорт
	...
	ИначеЕсли ИмяМетода = "РасчетныеПартииПрочих" Тогда
		СформироватьПартииПрочихРасходов(ПараметрыРасчета, ДанныеДляПартий);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
