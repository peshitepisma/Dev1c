...
Процедура Использование_ИсправлениеДублейВидовЗапасов(ПараметрыОбработчика) Экспорт
	...
	Если РазмерВременнойТаблицы(Запрос, "ВтОтборыДублей", ПараметрыОбработчика) = 0 Тогда
...
Процедура Использование_РасчетПартийИСебестоимости(ПараметрыОбработчика) Экспорт
	...
	УвеличитьКоличествоОбработанныхДанныхДляЗамера(ПараметрыОбработчика, ПараметрыОбработчика.ДанныеЭтапа.МассивОрганизаций.Количество());
...
//
Функция ИнициализироватьПараметрыРегистрацииПроблемПроверки(ИмяВременнойТаблицы, ШаблонТекстаОшибки, СписокПолей, ИмяПоляОбъекта = "") Экспорт
	
	ПараметрыРегистрации = Новый Массив;
	
	ДополнитьПараметрыРегистрацииПроблемПроверки(ПараметрыРегистрации, ИмяВременнойТаблицы, ШаблонТекстаОшибки, СписокПолей, ИмяПоляОбъекта);
...
//
Процедура ДополнитьПараметрыРегистрацииПроблемПроверки(ПараметрыРегистрации, ИмяВременнойТаблицы, ШаблонТекстаОшибки, СписокПолей, ИмяПоляОбъекта = "") Экспорт
...
Процедура ПродублироватьВЖурналРегистрации(ЭтапРасчета, ИсточникОпределенияВажностиПроблемы, ТекстОшибки)
	...
	Если Важность = Перечисления.ВариантыВажностиПроблемыСостоянияСистемы.Ошибка
	 ИЛИ Важность = Перечисления.ВариантыВажностиПроблемыСостоянияСистемы.Предупреждение Тогда
		УровеньЖР = УровеньЖурналаРегистрации.Ошибка;
	ИначеЕсли Важность = Перечисления.ВариантыВажностиПроблемыСостоянияСистемы.ВажнаяИнформация Тогда
...
Функция ТекстыЗапросовДляПолученияТаблицыИзмененийРегистра(МетаданныеРегистра) Экспорт
	...
	Если ЭтоРегистрБухгалтерии Тогда
		...
		ТаблицаРегистра = ТаблицаРегистра + ".ДвиженияССубконто";
...
Процедура ДобавитьСоставнойОбъектКЗафиксированнойПроблеме(ПараметрыПроверки, ИсточникДанных, ОписаниеПроблемы, СписокПолей)
...
Функция СравнитьСостояниеЭтаповЗакрытияМесяцаДляТестирования(Период,
			КаталогДляВыгрузки1, КаталогДляВыгрузки2, НеВыводитьНовыеЭтапы = Ложь) Экспорт
	...
	Для Каждого ТекущийЭтап Из МассивЭтапов Цикл
		...
		Если НЕ ЗначениеЗаполнено(Состояние1) И НЕ ЗначениеЗаполнено(Состояние2) Тогда
			...
			Если СтрокаИсходнойТаблицы = Неопределено Тогда
				...
				ТекстыСообщений.Добавить(НСтр("ru='Ошибка: операция отсутствует в таблице выгрузки операций'"));
